import type { Condition, VariantType } from "../types";
import type { RarityTier } from "../rarity";

/**
 * คำแปลภาษาไทย — ไฟล์นี้เป็นต้นแบบของโครงสร้าง
 * ภาษาอื่นต้องมีคีย์ครบเท่านี้ ไม่งั้น TypeScript จะฟ้องตอน build
 */
export const th = {
  meta: {
    siteTitle: "Collection Card — ราคาการ์ดสะสม One Piece และ Pokémon",
    siteDescription:
      "ฐานข้อมูลราคาการ์ดสะสม One Piece และ Pokémon แยกตามชุด เวอร์ชัน และสภาพการ์ด พร้อมราคาย้อนหลัง",
  },

  nav: {
    home: "หน้าแรก",
    browse: "เลือกเกม",
    movers: "ราคาขยับแรง",
    dashboard: "แดชบอร์ด",
    start: "เริ่มใช้งาน",
    tagline: "ราคาการ์ดสะสม",
    portfolio: "พอร์ตของฉัน",
    login: "เข้าสู่ระบบ",
    logout: "ออกจากระบบ",
    register: "สมัครสมาชิก",
  },

  footer: {
    blurb:
      "ฐานข้อมูลราคาการ์ดสะสมสำหรับนักสะสมชาวไทย แยกราคาตามเวอร์ชันและสภาพการ์ดอย่างละเอียด",
    phase:
      "รายชื่อการ์ดมาจากเว็บทางการ One Piece Card Game · ราคายังเป็นข้อมูลสาธิต ไม่ใช่ราคาตลาดจริง",
  },

  landing: {
    eyebrow: "One Piece · Pokémon",
    headlineLead: "รู้ราคาการ์ดของคุณ",
    headlineAccent: "ก่อนจะซื้อหรือปล่อย",
    sub: "ฐานข้อมูลราคาการ์ดสะสมสำหรับนักสะสมชาวไทย แยกราคาตามชุด เวอร์ชัน และสภาพการ์ดอย่างละเอียด พร้อมกราฟย้อนหลังให้เห็นแนวโน้ม",
    ctaPrimary: "เริ่มใช้งาน",
    ctaSecondary: "ดูราคาขยับแรง",
    freeNote: "เปิดใช้ฟรี ไม่ต้องสมัครสมาชิก",
    showcaseLabel: "ขยับแรงที่สุดใน 7 วัน",

    statCards: "ใบการ์ดที่แยกราคา",
    statVariants: "เลขการ์ดทั้งหมด",
    statSets: "ชุดการ์ดในระบบ",
    statHistory: "วันของราคาย้อนหลัง",

    featuresEyebrow: "สิ่งที่ใช้งานได้แล้ววันนี้",
    featuresTitle: "ละเอียดพอที่จะเชื่อได้จริง",
    featuresSub:
      "เว็บราคาการ์ดส่วนใหญ่บอกราคาเดียวต่อการ์ดหนึ่งใบ ซึ่งใช้ตัดสินใจซื้อขายจริงไม่ได้ เพราะราคาขึ้นกับเวอร์ชันและสภาพเสมอ",

    features: [
      {
        title: "ราคาแยกตามเวอร์ชันและสภาพ",
        body: "การ์ดใบเดียวกัน Normal กับ Alt Art ราคาต่างกันได้สิบเท่า เราแยกให้ครบทุกเวอร์ชัน คูณด้วยสภาพการ์ดห้าระดับ",
        cta: "ดูตัวอย่าง",
      },
      {
        title: "กราฟราคาย้อนหลัง",
        body: "เห็นราคาย้อนหลัง 90 วันของทุกเวอร์ชัน รู้ว่ากำลังขึ้นหรือกำลังลงก่อนตัดสินใจ",
        cta: "ดูกราฟ",
      },
      {
        title: "จับการ์ดที่ราคาขยับแรง",
        body: "อันดับการ์ดที่ขึ้นและลงมากที่สุดในเจ็ดวัน อัปเดตตามข้อมูลจริงทุกครั้งที่มีการบันทึกราคา",
        cta: "ดูอันดับ",
      },
      {
        title: "แยกฉบับ JP และ EN",
        body: "ชุดเดียวกันคนละภาษาคือคนละตลาดคนละราคา เราเก็บแยกกันตั้งแต่ต้น ไม่เอามารวมให้สับสน",
        cta: "เลือกเกม",
      },
    ],

    portfolioChip: "ใช้งานได้แล้ว",
    portfolioTitle: "เก็บการ์ดที่คุณมีเข้าพอร์ตของตัวเอง",
    portfolioSub:
      "สมัครบัญชีแล้วบันทึกการ์ดที่คุณมีจริง ระบบคิดมูลค่ารวมให้จากราคาล่าสุดในเว็บ พร้อมกำไรขาดทุนเทียบราคาที่ซื้อมา — ส่วนการดูราคาและค้นหายังไม่ต้องล็อกอินเหมือนเดิม",
    portfolioPoints: [
      "บันทึกการ์ดที่มีจริง แยกตามสภาพและจำนวนใบ",
      "มูลค่าพอร์ตรวม คิดจากราคาล่าสุดของการ์ดแต่ละใบ",
      "กำไรขาดทุนเทียบกับต้นทุนที่กรอกไว้",
      "พอร์ตเป็นของส่วนตัว คนอื่นมองไม่เห็น",
    ],
    portfolioCta: "สร้างพอร์ตของฉัน",
    portfolioCtaSignedIn: "ดูพอร์ตของฉัน",
    nextUpLabel: "กำลังพัฒนาต่อ",
    nextUp:
      "มูลค่าพอร์ตย้อนหลัง · แจ้งเตือนเมื่อการ์ดที่หมายตาลงถึงราคาที่ตั้งไว้ · เพิ่มการ์ดด้วยการถ่ายรูป",

    finalTitle: "เริ่มดูราคาการ์ดได้เลยตอนนี้",
    finalSub: (cards: string, sets: number, price: string | null) =>
      price
        ? `มีข้อมูล ${cards} ใบจาก ${sets} ชุดให้ไล่ดู การ์ดที่แพงที่สุดในระบบตอนนี้อยู่ที่ ${price}`
        : `มีข้อมูล ${cards} ใบจาก ${sets} ชุดให้ไล่ดู ราคากำลังทยอยอัปเดตเข้าระบบ`,
  },

  browse: {
    title: "เลือกเกม",
    heading: "จะดูการ์ดเกมไหนดี",
    sub: "เลือกเกมเพื่อไล่ดูชุดทั้งหมด แล้วเจาะเข้าไปดูราคาของการ์ดแต่ละใบ",
    description: "เลือกเกมเพื่อดูชุดการ์ดทั้งหมดและราคาปัจจุบัน One Piece และ Pokémon",
    cards: "การ์ด",
    sets: "ชุด",
    updated: "อัปเดต",
    moversTitle: "ราคาขยับแรงใน 7 วัน",
    seeAll: "ดูทั้งหมด →",
  },

  game: {
    title: (game: string) => `ชุดการ์ด ${game}`,
    description: (game: string) =>
      `รายการชุดการ์ด ${game} ทั้งหมด พร้อมจำนวนการ์ดและวันวางจำหน่าย`,
    inDb: "ในฐานข้อมูล",
    empty: "ยังไม่มีชุดของเกมนี้ในระบบ",
    progressLabel: (pct: number) => `มีข้อมูลแล้ว ${pct}% ของชุด`,
  },

  set: {
    title: (code: string, name: string) => `${code} ${name} — ราคาการ์ดทั้งชุด`,
    description: (code: string, name: string) =>
      `ราคาการ์ดในชุด ${code} ${name} ทุกใบ อัปเดตล่าสุด แยกตามเวอร์ชันและสภาพการ์ด`,
    released: "วางจำหน่าย",
    rarity: "Rarity",
    sortBy: "เรียงตาม",
    all: "ทั้งหมด",
    byNumber: "เลขการ์ด",
    byPrice: "ราคาสูงสุด",
    byChange: "ขยับแรง",
    has: (variant: string) => `มี ${variant}`,
    empty: "ไม่มีการ์ดที่ตรงกับตัวกรองนี้",
  },

  card: {
    title: (number: string, name: string, price: string) =>
      `${number} ${name} — ราคาล่าสุด ${price}`,
    description: (name: string, nameEn: string, number: string, set: string) =>
      `ราคาการ์ด ${name} (${nameEn}) ${number} จากชุด ${set} แยกตามเวอร์ชันและสภาพการ์ด พร้อมราคาย้อนหลัง 90 วัน`,
    noPrice: "ยังไม่มีข้อมูล",
    conditionNm: "สภาพ NM",
    per7d: "/ 7 วัน",
    updated: "อัปเดต",
    historyTitle: "ราคาย้อนหลัง 90 วัน",
    historyLabel: (name: string) => `กราฟราคาย้อนหลัง 90 วันของ ${name}`,
    printing: "แบบพิมพ์",
    priceTableTitle: "ราคาแยกตามเวอร์ชันและสภาพ",
    psaLabel: "PSA 10",
    psaNote: "ใบส่งเกรดได้ 10 เต็ม",
    priceNote:
      "ราคาผูกกับเวอร์ชันการ์ดและสภาพ ไม่ใช่ผูกกับตัวการ์ด — คอลัมน์ PSA 10 คือใบที่ส่งเกรดแล้วได้ 10 เต็ม ซึ่งเป็นคนละตลาดกับการ์ดดิบ ราคาประเมินจากราคาการ์ดดิบบวกเบี้ยของแต่ละใบ โดยมีค่าส่งเกรดเป็นพื้น ส่วนเกรด BGS จะเพิ่มในเฟสถัดไป",
    siblings: (code: string) => `การ์ดอื่นในชุด ${code}`,
    notEnoughData: "ยังมีข้อมูลราคาไม่พอสำหรับวาดกราฟ",
    low: "ต่ำสุด",
    high: "สูงสุด",
    days: (n: number) => `${n} วัน`,
  },

  /** ช่องทางราคา — ชื่อร้านไม่แปล แต่คำอธิบายรอบ ๆ แปล */
  channel: {
    title: "ราคาตามช่องทาง",
    ebay: "eBay",
    snkrdunk: "SNKRDUNK",
    estimated: "ประเมิน",
    empty: "ยังไม่มีราคาที่บันทึกไว้",
    note: "ราคาจากช่องทางต่างประเทศต้องกรอกเองในแดชบอร์ด ยังไม่ได้ดึงอัตโนมัติ",
  },

  search: {
    title: "ค้นหา",
    heading: "ค้นหาการ์ดจากชื่อ",
    placeholder: "ชื่อการ์ด หรือเลขการ์ด",
    submit: "ค้นหา",
    hint: "พิมพ์อย่างน้อย 2 ตัวอักษร ค้นได้ทั้งชื่อไทย ชื่ออังกฤษ และเลขการ์ด เช่น ลูฟี่ · Shanks · OP01-120",
    noResults: (q: string) => `ไม่พบการ์ดที่ตรงกับ "${q}"`,
    found: (n: number) => `พบ ${n} ใบ`,
  },

  movers: {
    title: "ราคาขยับแรงใน 7 วัน",
    description:
      "การ์ดสะสม One Piece และ Pokémon ที่ราคาขึ้นและลงมากที่สุดในรอบ 7 วัน",
    eyebrow: "7 วันล่าสุด",
    heading: "ราคาขยับแรง",
    sub: "เรียงตามขนาดการเปลี่ยนแปลงของราคา ไม่ว่าจะขึ้นหรือลง คิดจากราคาสภาพ NM",
    colRank: "#",
    colCard: "การ์ด",
    colSet: "ชุด",
    colVariant: "เวอร์ชัน",
    colPrice: "ราคา",
    colChange: "7 วัน",
    empty: "ยังไม่มีราคาในระบบ — กรอกราคาในแดชบอร์ดก่อน แล้วอันดับนี้จะขึ้นเอง",
  },

  auth: {
    loginTitle: "เข้าสู่ระบบ",
    loginHeading: "เข้าสู่ระบบ",
    loginSub: "เข้าสู่ระบบเพื่อเปิดพอร์ตการ์ดของคุณเอง — ดูราคาและค้นหาไม่ต้องล็อกอินก็ได้",
    registerTitle: "สมัครสมาชิก",
    registerHeading: "สร้างบัญชีใหม่",
    registerSub:
      "บัญชีมีไว้ผูกพอร์ตการ์ดเข้ากับตัวคุณเท่านั้น ข้อมูลราคาทั้งเว็บยังเปิดให้ทุกคนดูฟรีเหมือนเดิม",
    email: "อีเมล",
    displayName: "ชื่อที่ใช้แสดง",
    password: "รหัสผ่าน",
    passwordHint: (min: number) => `อย่างน้อย ${min} ตัวอักษร`,
    submitLogin: "เข้าสู่ระบบ",
    submitRegister: "สมัครสมาชิก",
    working: "กำลังดำเนินการ…",
    noAccount: "ยังไม่มีบัญชี?",
    haveAccount: "มีบัญชีอยู่แล้ว?",
    signedInAs: "เข้าสู่ระบบในชื่อ",

    forgotLink: "ลืมรหัสผ่าน?",
    forgotTitle: "ลืมรหัสผ่าน",
    forgotHeading: "ตั้งรหัสผ่านใหม่",
    forgotSub: "กรอกอีเมลที่ใช้สมัคร แล้วเราจะส่งลิงก์ตั้งรหัสผ่านใหม่ให้ ลิงก์ใช้ได้ 1 ชั่วโมง",
    forgotSubmit: "ขอลิงก์ตั้งรหัสใหม่",
    forgotSent: "ถ้าอีเมลนี้มีบัญชีอยู่ เราได้ส่งลิงก์ตั้งรหัสผ่านใหม่ไปให้แล้ว",
    forgotSentNoMail:
      "รับเรื่องแล้ว แต่เว็บนี้ยังไม่ได้ต่อระบบส่งอีเมล ให้ติดต่อผู้ดูแลเว็บเพื่อขอลิงก์ตั้งรหัสใหม่",
    backToLogin: "← กลับไปหน้าเข้าสู่ระบบ",

    resetTitle: "ตั้งรหัสผ่านใหม่",
    resetHeading: "ตั้งรหัสผ่านใหม่",
    resetSub: "ตั้งรหัสผ่านใหม่แล้วจะเข้าสู่ระบบให้ทันที ลิงก์เดิมจะใช้ไม่ได้อีก",
    resetSubmit: "บันทึกรหัสผ่านใหม่",
    newPassword: "รหัสผ่านใหม่",
    resetInvalidTitle: "ลิงก์นี้ใช้ไม่ได้แล้ว",
    resetInvalidSub:
      "ลิงก์ตั้งรหัสผ่านใหม่มีอายุ 1 ชั่วโมง และใช้ได้ครั้งเดียว ขอลิงก์ใหม่อีกครั้งได้เลย",
    resetAskAgain: "ขอลิงก์ใหม่",
  },

  portfolio: {
    title: "พอร์ตการ์ดของฉัน",
    description: "รวมการ์ดที่คุณมีจริง พร้อมมูลค่าปัจจุบันและกำไรขาดทุนเทียบราคาที่ซื้อมา",
    heading: "พอร์ตของฉัน",
    sub: "บันทึกการ์ดที่คุณมีจริง แล้วดูมูลค่ารวมเทียบกับราคาที่ซื้อมา ราคาคิดจากข้อมูลล่าสุดในเว็บ",
    empty: "พอร์ตยังว่างอยู่",
    emptySub: "เปิดหน้าการ์ดใบไหนก็ได้ แล้วกด “เพิ่มเข้าพอร์ต” เพื่อเริ่มเก็บ",
    emptyCta: "ไปหาการ์ด",
    statValue: "มูลค่าปัจจุบัน",
    statCost: "ต้นทุนรวม",
    statGain: "กำไร / ขาดทุน",
    statCards: "จำนวนใบ",
    noCostYet: "ต้องมีทั้งต้นทุนและราคาล่าสุดถึงคิดให้ได้",
    colCard: "การ์ด",
    colCondition: "สภาพ",
    colQty: "จำนวน",
    colCost: "ต้นทุน/ใบ",
    colPrice: "ราคาล่าสุด/ใบ",
    colValue: "มูลค่ารวม",
    colActions: "",
    remove: "เอาออก",
    save: "บันทึก",
    missingCard: "การ์ดใบนี้ถูกลบออกจากระบบแล้ว",
    noPrice: "ยังไม่มีราคา",
    addTitle: "เพิ่มเข้าพอร์ต",
    addSub: "เก็บไว้ในพอร์ตของคุณเอง คนอื่นมองไม่เห็น",
    condition: "สภาพ",
    quantity: "จำนวน (ใบ)",
    cost: "ราคาที่ซื้อมา ต่อใบ (บาท)",
    costHint: "เว้นว่างได้ แต่จะคิดกำไรขาดทุนให้ไม่ได้",
    note: "โน้ต",
    noteHint: "เช่น ซื้อจากใคร เก็บไว้ที่ไหน",
    add: "เพิ่มเข้าพอร์ต",
    added: "เพิ่มเข้าพอร์ตแล้ว",
    removed: "เอาออกจากพอร์ตแล้ว",
    updated: "บันทึกแล้ว",
    loginToAdd: "เข้าสู่ระบบเพื่อเก็บการ์ดใบนี้เข้าพอร์ตของคุณ",
    viewPortfolio: "ดูพอร์ตของฉัน",
    guestNote: "ดูราคาและค้นหาได้โดยไม่ต้องล็อกอิน — บัญชีมีไว้สำหรับเก็บพอร์ตเท่านั้น",
  },

  variant: {
    normal: "Normal",
    parallel: "Parallel",
    alt_art: "Alt Art",
    manga: "Manga Rare",
    full_art: "Full Art",
    promo: "Promo",
  } satisfies Record<VariantType, string>,

  /** ประเภทการ์ด — ใช้คำเดียวกับที่เว็บทางการภาษาไทยใช้ */
  cardType: {
    Leader: "ลีดเดอร์",
    Character: "คาแรกเตอร์",
    Event: "อีเวนต์",
    Stage: "สเตจ",
  } as Record<string, string>,

  /** สีการ์ด — ข้อมูลเก็บเป็นภาษาไทยอยู่แล้ว ตรงนี้จึงเป็นตัวเดิม */
  color: {
    แดง: "แดง",
    เขียว: "เขียว",
    ฟ้า: "ฟ้า",
    ม่วง: "ม่วง",
    ดำ: "ดำ",
    เหลือง: "เหลือง",
    ไม่มีสี: "ไม่มีสี",
  } as Record<string, string>,

  tier: {
    mythic: "หายากที่สุด",
    epic: "หายากมาก",
    rare: "หายาก",
    common: "ทั่วไป",
  } satisfies Record<RarityTier, string>,

  condition: {
    PSA10: "PSA 10",
    NM: "NM",
    LP: "LP",
    MP: "MP",
    HP: "HP",
    DMG: "DMG",
  } satisfies Record<Condition, string>,

  time: {
    justNow: "เมื่อครู่",
    minutes: (n: number) => `${n} นาทีที่แล้ว`,
    hours: (n: number) => `${n} ชม.ที่แล้ว`,
    days: (n: number) => `${n} วันก่อน`,
  },
};

export type Dictionary = typeof th;
