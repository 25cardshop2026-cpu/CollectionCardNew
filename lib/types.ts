...
export interface Card {
  ...
  sourceUrl?: string;
  /**
   * เลขสินค้าบน SNKRDUNK (ตัวเลขท้าย URL หน้าสินค้า เช่น .../trading-cards/864495 → "864495")
   * มี = ให้ job ซิงก์ราคาอัตโนมัติดึงราคาต่ำสุดปัจจุบันมาอัปเดตช่อง SNKRDUNK ให้ใบนี้
   * ไม่มี = ข้ามใบนี้ไปเฉย ๆ — นี่คือกลไก "เลือกเฉพาะสินค้าที่ต้องการ" ทั้งหมด
   */
  snkrdunkCode?: string;
  /**
   * เวลาที่ลองซิงก์ราคาจาก SNKRDUNK ล่าสุด ไม่ว่าจะได้ราคากลับมาหรือไม่ก็ตาม
   * ต่างจากเวลาบันทึกราคาจริง — มีไว้กันใบที่ยังไม่มีคนลงขายเลยค้างอยู่หัวคิว
   * ตลอดไป (ไม่เคยได้ราคาจึงไม่เคยขยับคิว) จนใบอื่นที่มีของขายจริงไม่ได้คิวสักที
   */
  snkrdunkCheckedAt?: string;
}
